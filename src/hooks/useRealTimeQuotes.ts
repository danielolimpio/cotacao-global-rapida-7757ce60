...
const fetchQuotes = async () => {
    try {
      ...
      // Try to fetch from a free financial API (Binance for crypto, other sources for forex)
      try {
        for (const [symbol, apiSymbol] of Object.entries(symbolMap)) {
          if (symbols.includes(symbol)) {
            let apiUrl = '';
            
            if (symbol.includes('BTC') || symbol.includes('ETH')) {
              // Binance API for crypto (free, no auth required)
              apiUrl = `https://api.binance.com/api/v3/ticker/24hr?symbol=${apiSymbol}`;
            } else {
              // For forex, we'll use exchangerate-api (free tier)
              ...
              if (symbol.includes('BTC') || symbol.includes('ETH')) {
                ...
                if (data.rates && data.rates[quoteCurrency]) {
                  ...
                  mockData[symbol] = {
                    symbol,
                    price: rate,
                    change: rate * 0.001, // Approximate change
                    changePercent: 0.1,   // Approximate percentage
                    high: rate * 1.01,
                    low: rate * 0.99
                  };
                  realDataFetched = true;
        ...
        console.warn('Failed to fetch real data, falling back to simulation:', apiError);
      }
